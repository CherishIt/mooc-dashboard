...
component('learnerGraph', {
  ...
  controller: function commentNetworkController($http, $scope, $routeParams,API_BASE_URL) {
    ...
    var charts = {};
    var chart = echarts.init(document.getElementById('learner_network'));
    charts.learner_network = chart;
    ...
    $http.get(API_BASE_URL+'courses/' + $routeParams.course_code +
      ...
      response.data.density.forEach(function(n) {
        ...
        d.push({
          name: t.toString(),
          value: [
            [t.getFullYear(), t.getMonth() + 1, t.getDate()].join('/'), n.density
          ]
        })
      });
      ///// for density
      var d_chart = echarts.init(document.getElementById('density'));
      charts.density = d_chart;
      var d_options = {
        title: {
